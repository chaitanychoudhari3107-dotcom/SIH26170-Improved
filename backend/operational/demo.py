...
The fixed, public training fixture is analysed in an in-memory database. This
does not add runs to the operator's database and does not use hidden labels.
"""
import csv
import sqlite3
...
def demo_result():
    ...
    try:
        ...
        return {'fixture': 'DEMO_A · public synthetic training fixture', 'lot_size': len(rows),
                ...
                'cases': cases,
                'limitations': 'These training and constructed fixtures have no independently verified defect labels; cases illustrate workflow, not accuracy.'}
    finally:
        connection.close()
