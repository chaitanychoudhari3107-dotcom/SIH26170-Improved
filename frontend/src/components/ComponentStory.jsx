  ...
  return <section className="bt-story" aria-labelledby="bt-story-title">
    ...
    <div className="bt-instrument">
      ...
      {item && measurement && <>
        ...
        <div className="bt-evidence-explain"><p><strong>Module A · lot-relative screen:</strong> {item.at_24h.module_a_disposition}. It checks this part against its lot; observed specification breaches are handled by the combined decision rule.</p><div className="bt-reason-heading">Module B · 168h forecast indicators</div><ModuleBReasons codes={item.at_24h.module_b_reason_codes} compact /><p>The combined QA action above uses all six parameters, not only the selected chart. {item.synthetic_label && item.at_24h.disposition === 'PROVISIONAL_PASS' ? 'This planted defect is a documented 24h miss; provisional pass is not final clearance.' : ''}</p></div>
