...
**Public evaluator path:** open the homepage and click **Open public guided demo**. It runs five synthetic cases through the backend decision path without a password or a CSV. Pick a case, inspect the 0h/24h measurements and 168h forecast, then reveal retrospective observations. The demo is read-only and is not evidence of accuracy on real hardware.

**Protected operator path:** the key unlocks CSV import, analysis, reviews and QA labels. The measurement template and QA outcome template are blank header-only files by design; use the provided 24h/168h fixture to rehearse import. The QA outcomes upload needs a component already imported into a lot, a DEFECTIVE or HEALTHY result, and a QA evidence reference. Validation precedes import. A confirmed suspected miss also becomes a labelled defect. Export QA labels + measurements before redeploying an instance without persistent storage.

Publishing the shared operator key in a publicly circulated PPT grants whoever has that deck write access to the demo workspace. Prefer the public read-only walkthrough for judges. If interactive operator access is necessary, use a disposable key and a separate demo service with no valuable records, then rotate the key after judging.
